...
// Generate video with text overlay (simulated - returns analysis text)
function generateVideoWithTextOverlay(analysis, keywordResults, originalVideoBuffer) {
  // In a real implementation, you would use FFmpeg to add text overlay to video
  // Since FFmpeg isn't available on Vercel, we return the analysis text
  // and the frontend can handle it or we return a modified video buffer
  
  const analysisText = `
🎯 VEHICLE DIAGNOSTIC REPORT
============================

🚗 MAIN ISSUE: ${analysis.mainProblem}
⚡ SEVERITY: ${analysis.severity.toUpperCase()}
🔧 PROBLEM TYPE: ${analysis.problemType}

📋 DETECTED ISSUES:
${keywordResults.foundKeywords.map((keyword, index) => `• ${keyword}`).join('\n')}

🔍 SPECIFIC PROBLEMS:
${analysis.specificIssues.map((issue, index) => `• ${issue}`).join('\n')}

💡 RECOMMENDATION:
${analysis.recommendation}

✅ Generated by Vehicle Diagnostic AI
📅 ${new Date().toLocaleString()}
  `.trim();

  // For now, we return the original video buffer and analysis text
  // In production, you'd process the video with FFmpeg
  return {
    videoBuffer: originalVideoBuffer, // Return original since we can't process on Vercel
...
app.post("/process-recording", upload.single("recording"), async (req, res) => {
  try {
    ...
    try {
      ...
      const videoResult = generateVideoWithTextOverlay(analysis, keywordResults, req.file.buffer);
      ...
      const response = {
        ...
        analysis: {
          ...
          // Return the analysis text for frontend display
          analysisText: videoResult.analysisText,
          // Return video data for download (original for now)
          enhancedVideo: {
            data: videoResult.videoBuffer.toString('base64'),
...
app.get("/", (req, res) => {
  res.json({
    ...
    features: ["Live recording", "Manual upload", "Keyword search", "AI analysis", "Enhanced video download"],
